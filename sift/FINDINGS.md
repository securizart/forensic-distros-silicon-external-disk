...
Empirically confirm the GIFT/libyal risk (v0.4+ on the project roadmap) — likely reproducing the same one-by-one test pattern used with REMnux (snapshot before each package, `salt-call --local state.apply <path> test=True` first, then the real run).
