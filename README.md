...
Validating each forensic distro on arm64 needs its own test cycle (VM, snapshots, binary-by-binary verification) before it can be reliably integrated into the bare-metal installer. This repo collects that validation work; once mature, it gets merged into the main project.

## Status per distro (v0.2)
...
**Done in v0.2**: REMnux install flow empirically validated on a VM (`install.sh` → `cleanup.sh` → `verify.sh`), with confirmed native arm64 alternatives for 4 of the 6 broken binaries.

- **v0.3**: close `cutter` in REMnux (build from source or via `box64`, no confirmed native alternative for now), finish the fine-grained triage still pending (NO-PKG + npm), and start the first phase of CAINE — extracting the tool list and cataloging it (which are plain portable apt packages/scripts vs. amd64-only binaries vs. tools with an available arm64 build).
- **v0.4+**: empirical testing of the GIFT/libyal family for SIFT.
