...
for name in "${!BROKEN_BINARIES[@]}"; do
    ...
    if echo "$elf_out" | grep -qiE 'x86[_-]64'; then
        echo "  [BAD]  $name: installed at $path but is x86-64 (Exec format error expected)"
        fail=1
    elif echo "$elf_out" | grep -qiE 'aarch64|arm64'; then
        echo "  [INFO] $name: installed at $path and IS arm64 (upstream patch? check)"
...
if command -v r2 >/dev/null 2>&1; then
    if r2 -v >/dev/null 2>&1; then
        ...
        echo "  [BAD]  radare2 installed but fails to run"
        fail=1
    fi
else
    echo "  [WARN] radare2 is not installed (check install.sh's log)"
fi
...
# =============================================================================
# ALWAYS run, no flags needed: after the verification above, this script
# directly installs docker-compose, redress, yr and die (the 4 confirmed
# working). cutter is skipped by default (no confirmed native
...
#
# Each binary uses whichever method fits it, so as not to drag in broken
# dependencies or clobber system packages:
#   - docker-compose: native Ubuntu package (apt, no external repos)
#   - cutter:         the project's own official repo (RizinOrg), pinned
#                      to low priority so it NEVER competes with
#                      Ubuntu's own archive for other packages (same
#                      pattern as Kali's pin in the base installer)
#   - redress:        native toolchain (apt's golang-go) + its own
#                      build; touches nothing beyond compiling
#   - yr (yara-x):    rustup (an isolated Rust toolchain under $HOME),
#                      NOT apt's cargo (too old for yara-x-cli)
#   - inspircd:       ONLY behind its own separate flag — Ubuntu's
#                      version (3.17.0) is 2 majors older than the
#                      4.7.0 remnux asks for, not a clean substitute
#   - die/diec:       official Flatpak (Flathub), isolated from the
#                      system by design (touches no apt dependencies
#                      at all)
# =============================================================================
...
install_docker_compose() {
    ...
    echo "      Symlink created. Try: docker-compose version  (or: docker compose version)"
}

install_cutter() {
    echo "[2/6] cutter -> cutter-re (RizinOrg's official repo, arm64, xUbuntu_24.04 build)"
    echo "      WARNING: the xUbuntu_22.04 build fails on Ubuntu 24.04 because"
    echo "      it depends on libpython3.10 (24.04 ships libpython3.12). Using"
    echo "      the xUbuntu_24.04 folder of the same OBS repo instead, not 100%"
    echo "      confirmed from here (no direct access to the repo) — if it also"
    echo "      fails, the alternative is building Cutter from source."
    echo "      The repo is pinned at Pin-Priority 100 so it never replaces"
    echo "      packages from Ubuntu's own main archive."
...
install_redress() {
    ...
    echo "      Installed at /usr/local/bin/redress. Try: redress version"
}
...
install_inspircd_downgrade() {
    ...
    if [ "$ans" = "y" ] || [ "$ans" = "Y" ]; then
        ...
        echo "      v3.17.0 installed — review the config by hand."
    else
        echo "      Skipped."
    fi
}
...
WRAPPER
    ...
    echo "      Installed as 'die' (a wrapper around the Flatpak)."
    echo "      WARNING: not confirmed whether the Flatpak also exposes 'diec'"
    echo "      (console variant) as a separate command — check manually with:"
    echo "      flatpak run --command=diec io.github.horsicq.detect-it-easy --help"
}
...
install_arm64_alternatives() {
    ...
    echo "(RizinOrg's repo tried on both xUbuntu_22.04 and xUbuntu_24.04, both fail)."
    echo "To try it anyway: ./verify.sh --with-cutter"
    echo ""
