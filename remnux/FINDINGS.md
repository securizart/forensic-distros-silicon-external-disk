...
| `die`/`diec` | Official Flatpak (`io.github.horsicq.detect-it-easy`, Flathub, `aarch64` supported) | ✅ Confirmed (launches the real Qt GUI; the `qt.core.qobject.connect` warnings come from DIE itself, not from architecture) | Not confirmed whether the Flatpak also exposes `diec` (console variant) as a separate command |
...
`remnux/exclude-list.txt` holds the `.sls` paths (dotted-path) categorized during the investigation of the ~120 failures from the full `remnux.addon` run. **No longer passed to `state.apply` as `exclude=`** (see the Important finding above); kept as reference for what fails and why. The actual cleanup of broken binaries is done by `cleanup.sh` after a full run with no exclusions.
